...
import CachedRequestsManager from "./models/CachedRequestManager.js";
...
export default class Response {
    ...
    JSON(obj, ETag = "", fromCache = false) {                         // ok status with content
        if (this.HttpContext.isCachable)
        {
            if (!fromCache)
                {
                    if (this.HttpContext.path.isAPI && CachedRequestsManager.exists(obj.url) == null)
                    {
                        console.log("je suis add");
                        CachedRequestsManager.add(obj.url, obj.content, obj.ETag);
                    }
